...
Как узнать `ADMIN_CHAT_ID`: временно оставить эту строку пустой, запустить бота
(шаг 3), отправить ему `/start` — он покажет ваш chat_id прямо в чате. Вписать
его сюда и перезапустить (шаг 4).
