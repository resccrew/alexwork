...
Можно вписать несколько человек через запятую — **у каждого будут полностью
свои данные** (свои смены, свой профиль/ставка, свой Excel-отчёт), общий
только сам бот и список отделений:
...
(шаг 4) — для второго и последующих пользователей больше ничего не нужно, их
данные создаются пустыми при первом обращении.

### Разовая миграция при добавлении первого дополнительного пользователя

Если база уже существует с версии, когда данные были общими на всех (до
разделения по пользователям), при первом перезапуске после обновления кода
нужно один раз указать, кому принадлежит вся старая история — иначе она
останется ничьей. Добавить в `.env` **только на этот единственный перезапуск**:

```
LEGACY_DATA_OWNER_CHAT_ID=<chat_id того, кто был единственным пользователем>
```

После успешного перезапуска (`sudo systemctl restart dyzury-bot medapp-api`)
эту строку можно убрать из `.env` — миграция одноразовая и безопасно
повторяется (при отсутствии `LEGACY_DATA_OWNER_CHAT_ID` просто ничего не
делает повторно).
